...
If you visited my website, you probably saw the Aurora behind it: three glowing bands of color that drift and blend like a CRT-suffused aurora borealis, with a halo chasing your cursor and a faint particle field that scatters as you move. [This page is darker than my first attempt](https://github.com/austinatchley/hugo-theme-hello-friend-ng) on purpose.

What you can't see from the outside is the process. I built that background over about twelve weeks of close pair-programming with an AI coding agent, and the whole collaboration is still sitting in the git history, commit by commit, if you care about that kind of thing.

This is a series about that workflow. It's about how a static site became a small living system, and what it meant to co-build it with an agent that can generate code faster than I can review it.
...
The fastest part of this whole setup isn't the code generation. It's the pipeline. A push to `main` and I get the result on the live site almost immediately.
...
That minute matters more than it sounds. It means the agent can try ideas locally with the dev server, and if a change is straightforward enough, the feedback loop is: commit, push, one minute, look at the real site. Cutting the lead time on a new feature down to that length changes how you choose what to try. Experiments that would feel too expensive at a day-long deploy cycle are free at a minute-long one.

The two workflows also enforce the split at the seam: the theme is guarded by its own quality gates, and the main repo has a Hugo-only build that needs no Node toolchain at all because the compiled JavaScript is committed.

## The open-source setup

Worth naming the toolchain, because the cost model shaped everything. The agent runs through [opencode](https://opencode.ai), an open-source terminal coding agent, and [OpenRouter](https://openrouter.ai), which routes requests to whatever model I want and lets me hot-swap mid-session. No proprietary closed-source harness, no subscription, and if a model goes sideways mid-task I switch to another one in a keystroke.
